...
- **`web/`** — React/Vite/TS. **`configuracao/` é a fronteira de cliente**: o esquema Zod
  ...
  **Camada nova: pergunte onde ela NÃO existe.** Cobertura parcial mente por omissão —
  ligada, pintando nada, sem erro. Já acontece: o `bairro` do IBGE existe em 895 dos
...
**188–892 ms** (mediana 361 ms), mas o município de São Paulo inteiro não terminou em 90 s.
A curva posterior encontrou o corte:
49,949 km² responderam em 787 ms; 78,045 km², em 979 ms, já sem folga. Desde a publicação
do commit `a65d377` nos dois clientes, agente e frontend recusam acima de **50 km²
...
### CNES e Inep no geodata (2026-09-18; réplica na VPS em 2026-09-19)

As cargas `CNES_NO_GEODATA` e `INEP_NO_GEODATA` persistiram o cadastro oficial no
PostGIS. A réplica na VPS subiu em 2026-09-19. Artefatos em
`.claude/sdd/archive/CNES_NO_GEODATA/` e `.claude/sdd/archive/INEP_NO_GEODATA/`.
...
O consumo no mapa, no Raio-X e no agente é a `EQUIPAMENTOS_OFICIAIS_NO_MAPA`
(2026-09-19): camadas `escolas` e `saude`, cartão do Raio-X com `versao_calculo` 6,
tool `equipamentos_no_ponto`. O de-para de `tipo_unidade` mora em
