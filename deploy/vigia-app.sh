...
# o host de tiles e infraestrutura compartilhada e o site e de um cliente. Quando
# houver um segundo cliente, ele ganha o proprio vigia e o dos tiles continua um.
#
...
SITE="${SITE_URL:-https://geo-intelligence.averisen.com}"
falhas=()
...
if [ ${#falhas[@]} -eq 0 ]; then
    echo "$(date '+%F %T') app ok"
    [ "${1:-}" = "--sempre" ] && avisar "geo: ok" "Site e agente saudaveis em $(date '+%F %H:%M')" default white_check_mark
    exit 0
fi

printf '%s FALHA\n%s\n' "$(date '+%F %T')" "$(printf '%s\n' "${falhas[@]}")"
avisar "geo: FALHA" "$(printf '%s\n' "${falhas[@]}")" urgent rotating_light
exit 1
